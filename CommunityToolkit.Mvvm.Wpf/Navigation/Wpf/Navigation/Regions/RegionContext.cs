...
namespace CommunityToolkit.Mvvm.Navigation.Regions
{
    ...
    public static class RegionContext
    {
        private static readonly DependencyProperty ObservableRegionContextProperty =
            DependencyProperty.RegisterAttached("ObservableRegionContext", typeof(ObservableCtsAccessor<object>), typeof(RegionContext), null);

        /// <summary>
        /// Returns an <see cref="ObservableCtsAccessor{T}"/> wrapper around the RegionContext value. The RegionContext
        ...
        ///
        /// If the <see cref="ObservableCtsAccessor{T}"/> wrapper does not already exist, an empty one will be created. This way, an observer can
        ...
        public static ObservableCtsAccessor<object> GetObservableContext(DependencyObject view)
        {
            if (view == null)
                throw new ArgumentNullException(nameof(view));

            ObservableCtsAccessor<object> context = view.GetValue(ObservableRegionContextProperty) as ObservableCtsAccessor<object>;

            if (context == null)
            {
                context = new ObservableCtsAccessor<object>();
